...
- If multiple rows share the same percentage, insert the new row **before** the first existing row with the same value (newest result goes on top within a tied group).
- If the new row has no agentic result, sort it among the bottom rows by the Chat Based Tests value (descending), newest first within tied values.
